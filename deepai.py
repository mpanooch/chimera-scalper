...
class DemoTrainingSystem:
    ...
    def generate_signal(self, symbol, price):
        ...
        # Generate signal based on regime with higher threshold
        signal = None
        if regime in [1, 3] and confidence > 0.85:  # BULL or BREAKOUT - high confidence only
            signal = {
                'action': 'BUY',
        ...
        elif regime in [0, 4] and confidence > 0.85:  # BEAR or REVERSAL - high confidence only
            signal = {
                'action': 'SELL',
                'confidence': confidence,
                'regime': regime_name,
