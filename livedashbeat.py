...
class DashboardBridge:
    ...
    async def send_trade_signal(self, trade_data):
        try:
            message = json.dumps({
                'type': 'trade_signal',  # Changed to match server.js expectation
                'data': {
...
# Change this line in __init__:
        self.ws_url = "wss://stream.bybit.com/v5/public/spot"  # Instead of "linear"
    ...
    def process_ticker(self, symbol, ticker_data):
        ...
        # Debug: Print raw data structure every 50 ticks to see what we're actually getting
        if self.stats['ticks'] % 50 == 0:
            print(f"DEBUG {symbol} raw data: {ticker_data}")

        # Extract data - try multiple field names in priority order
        price = 0

        # Priority 1: lastPrice (most accurate)
        ...
        # Priority 2: If no lastPrice, use average of bid/ask
        if price <= 0 and 'bid1Price' in ticker_data and 'ask1Price' in ticker_data:
            try:
        ...
        # Priority 3: Use markPrice as fallback
        if price <= 0 and 'markPrice' in ticker_data:
            try:
        ...
        if 'price24hPcnt' in ticker_data:
            try:
                change_pct = float(ticker_data['price24hPcnt']) * 100  # Convert to percentage
            except (ValueError, TypeError):
                change_pct = 0
    ...
    def generate_signal(self, symbol, price):
        ...
        # Make prediction - EXPLICITLY set to eval mode
        self.model.eval()
        with torch.no_grad():
            ...
            async with websockets.connect(self.ws_url) as ws:
                ...
                while True:
                    try:
                        ...
                        # Handle subscription success/error responses
                        if 'success' in data:
                            if data['success']:
                                print(f"✅ Subscription successful: {data.get('ret_msg', 'OK')}")
                            else:
                                print(f"❌ Subscription failed: {data.get('ret_msg', 'Unknown error')}")
                        ...
                        if 'topic' in data and 'data' in data:
                            ...
                            # Show data received indicator
                            if current_time - last_data_received > 10:  # Every 10 seconds
                                print(f"📊 Data flowing: {symbol} @ ${float(ticker_data.get('lastPrice', 0)):,.2f}")
