...
echo "🔧 Fixing std::atomic<float> errors"
echo "===================================="

# Fix layer5_execution.cpp - atomic<float> doesn't support += operator
...
echo "✓ Fixed layer5_execution.cpp"

# Now rebuild
echo ""
echo "🔨 Rebuilding CHIMERA..."
cd build
make -j$(nproc)
...
if [ -f "bin/chimera_scalper" ]; then
    ...
    echo ""
    echo "Still have errors. Let me check..."
    make 2>&1 | grep error
fi